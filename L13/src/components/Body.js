import Card,{CardsWithPermoted} from "./Card";
...
const Body = ()=>{
    ...
    const RestCardPermoted           = CardsWithPermoted(Card);  //Higher order component
    ...
    return (
        <div className="my-5">
            ...
            <input type="text" placeholder="Enter User Name"  className='py-2 px-2 rounded-md  text-black font-sans font-medium border-2 border-black ' onChange={(e)=>UserData.setUserName(e.target.value)} value={UserData.loggedInUser} maxLength={10} cla></input>
            ...
            {   
                ...
                return <Link key={restaurent.id} to={"/restaurant/"+restaurent.id}>
                            {restaurent.permoted?<RestCardPermoted resdata={restaurent}></RestCardPermoted>:<Card resdata={restaurent}></Card>}
                        </Link>
                })
            }
            </div>
